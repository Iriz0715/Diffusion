...
def main(config):
    ...
    if final:
        ...
        train_data_dir = identifier + '/data_training'
        ...
        train_data_dir = dir + '/data_training/train/train'
        output_path = identifier + '/output/cv/' + os.path.basename(deploy_input.strip('/'))
    
    if deploy_input == '':
        deploy_input = identifier + '/data_training/test'
    ...
    ########## 20250630 diffusion
        ...
        model_config = {
            'epoch': 10, 
            'batch_size': 1,
            'iters_per_epoch': 10,   # 每个epoch的迭代次数
            'save_period': 10,
            'optimizer_type': 'Adam',
            # 'initial_lr': 1e-4,
            'norm_config': {'norm': False, 'norm_channels': [0],
    ...
    if is_train:
        ...
        if model_name == 'DDPM':
            model.diffusion_test(validation_paths, output_path, squeue=500)
        else:
