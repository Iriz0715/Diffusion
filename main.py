...
parser.add_argument('-m', '--model', type=str, default='DDPM3d', help='Model name [UNet2D, CycleGAN, DDPM2d, DDPM3d]')
...
def main(config):
    ...
    if model_name not in ['UNet2D', 'CycleGAN','DDPM2d','DDPM3d']:
        raise Exception('Unsupported model')
    
    if final:
        checkpoint_dir = identifier + '/checkpoint_2d_patch'
    ...
    if deploy_input == '':
        #deploy_input = dir + '/data_training/test_2'
        deploy_input = identifier + '/data_training' # train data测试
        valid_data_dir = dir + '/data_training/valid'
        #deploy_input = valid_data_dir
    ...
    validating_paths = [os.path.join(valid_data_dir, name) for name in os.listdir(valid_data_dir) if '.hdf5' in name and name.startswith('synthrad')]
    ...
    ########## 20250630 diffusion
        ...
        training_paths = [p for p in training_paths]
        testing_paths = [p for p in testing_paths]
        validation_paths = [p for p in validating_paths]
        #im_size = [256, 256]
        im_size = [8, 256, 256]   # 3d UNet
        ...
        model_config = {
            'epoch': 200, 
            'batch_size': 1,
            'iters_per_epoch': 500,   # 每个epoch的迭代次数
            'save_period': 20,
            'optimizer_type': 'Adam',
            'initial_lr': 1e-4,
            'norm_config': {'norm': False, 'norm_channels': [0],
    ...
    #    MainModel = CycleGAN
    elif model_name == 'DDPM2d':
        MainModel = DDPM_UNet2D
    elif model_name == 'DDPM3d':
        MainModel = DDPM_UNet3D
    else:
    ...
    if is_train:
        ...
        if identifier == 'DDPM':
            ...
            model.diffusion_test_batch(testing_paths=testing_paths, output_path=output_path, batch_size=1, sampler='ddim')
        else:
            model.test(validation_paths, output_path) # 之前的代码
