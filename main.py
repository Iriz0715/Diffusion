...
parser.add_argument('-m', '--model', type=str, default='DDPM2D', help='Model name [UNet2D, CycleGAN, DDPM2D]')
...
def main(config):
    ...
    if model_name not in ['UNet2D', 'CycleGAN','DDPM2D']:
    ...
    if deploy_input == '':
        valid_data_dir = dir + '/data_training/valid_5'
        deploy_input = dir + '/data_training/test'
        
    training_paths = [os.path.join(train_data_dir, name) for name in os.listdir(train_data_dir)
                      if name.endswith('.hdf5') and name.startswith('synthrad')]
    testing_paths = [os.path.join(deploy_input, name) for name in os.listdir(deploy_input)
                     if name.endswith('.hdf5') and name.startswith('synthrad')]
    validating_paths = [os.path.join(valid_data_dir, name) for name in os.listdir(valid_data_dir)
                        if name.endswith('.hdf5') and name.startswith('synthrad')]
    ...
    if is_train:
        ...
        if identifier == 'DDPM':
            model.diffusion_infer_global(testing_paths=testing_paths, output_path=output_path)
        else:
