...
class DDPMBaseModel2D(tf.keras.models.Model):
    ...
    def hdf5_generator(self, folder_path, target_size, model_mode='train'):
        ...
        for file_idx, file_path in enumerate(file_list):
            with h5py.File(file_path, 'r') as f_h5:
                ...
                input_imgs = input_imgs * 2. - 1.  ############ Scale to [-1, 1]
                output_imgs = output_imgs * 2. - 1. ############ Scale to [-1, 1]
                input_img = self.crop_pad3D(input_imgs, target_size)
                output_img = self.crop_pad3D(output_imgs, target_size)
                input_img = np.expand_dims(input_img, axis=-1)
                output_img = np.expand_dims(output_img, axis=-1)
                yield input_img, output_img, file_idx   # [32, 256, 256, 1]

    def get_tf_dataset(self, folder_path, target_size, model_mode='train', batch_size=8, shuffle=True):
        output_types = (tf.float32, tf.float32, tf.int32)
        output_shapes = (target_size + (1,), target_size + (1,), ())
        ds = tf.data.Dataset.from_generator(
            lambda: self.hdf5_generator(folder_path, target_size, model_mode),
    ...
    def validate_diffusion(self, validation_paths):
        ...
        for input_batch, target_batch, file_idx_batch in val_dataset:
            ...
            for i in range(input_batch.shape[0]):
                pred = pred_batch[i, ..., -1]   # [32, 256, 256]
                gt = gt_batch[i, ..., -1]
                mae = np.mean(np.abs(gt - pred))
                ssim_val = ssim(pred, gt, data_range=2.0, channel_axis=None)      # 注意，clip也要改
                mae_list.append(mae)
    ...
    def diffusion_test_batch(self, testing_paths, output_path, batch_size=4, sampler='ddpm', **kwargs):
        ...
        for idx in case_ids:
            pred_stack = np.array(pred_dict[idx])
            gt_stack = np.array(gt_dict[idx])
            input_stack = np.array(input_dict[idx][0])  # [32, 256, 256]
